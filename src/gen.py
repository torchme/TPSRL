...
=======
df = pd.read_csv(PATH_DF + "dist_vologda_matrix.csv", index_col="Unnamed: 0").iloc[
    :16, :16
]
>>>>>>> ac4efa6bd1b000010fc67f6ea824f78aa864f969:src/gen.py
...
def crossing_one_sample(firstSample, secondSample, P_CROSSOVER):
    ...
    firstCrossSameple += [
        i for i in secondSample[index_crossing:] if i not in firstCrossSameple
    ]
    ...
    secondCrossSameple += [
        i for i in firstSample[index_crossing:] if i not in secondCrossSameple
    ]
...
def get_sample_mutation(sample, N_MUTATIONS):
    ...
    mutations_indexes = random.sample(
        _sample, N_MUTATIONS
...
def get_inference(num_iter):
    (
        ...
        population_sample,
        population_sample_total_dist,
    ) = populationCreator(POPULATION_SIZE)
    total_df = make_df(
        population_sample_indexes, population_sample, population_sample_total_dist
    )
    # print(f'Генерация')
    # print()
    # print(total_df.head(3))
    sub_sample_size = POPULATION_SIZE // 2
    sub_sample_1 = total_df.sample(n=sub_sample_size)[
        "indexes"
    ].to_list()  # указать вручную 1/2 от POPULATION SIZE
    sub_sample_2 = total_df.sample(n=sub_sample_size)[
        "indexes"
    ].to_list()  # указать вручную 1/2 от POPULATION SIZE
    ...
    for num_iteration in range(num_iter):
        ...
        for i in range(len(sub_sample_1)):
            ...
            sub_sample_1[i], sub_sample_2[i] = crossing_one_sample(
                sub_sample_1[i], sub_sample_2[i], P_CROSSOVER
            )
            cross_population_sample_indexes.append(sub_sample_1[i])
            cross_population_sample_indexes.append(sub_sample_2[i])

        cross_population_sample = [
            count_sample_list(i) for i in cross_population_sample_indexes
        ]
        cross_population_sample_total_dist = [
            countTotalDistance(i) for i in cross_population_sample
        ]
